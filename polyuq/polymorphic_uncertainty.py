...
scipy.interpolate.NearestNDInterpolator.point_val = point_val
scipy.interpolate.NearestNDInterpolator.neighbor_dist = neighbor_dist
...
class PolyUQ(object):
    ...
    def sample_qmc(self, N_mcs_ale=100000, N_mcs_epi=1000, percentiles=(0.0001, 0.9999),
        ...
        logger.info(f"Generating size-({n_vars} x {N_mcs}) low-discrepancy sequences for all variables (scramble seed: {seed})... ")
        ...
        logger.debug('Scaling sequences...')
    ...
    def estimate_imp(self, interp_fun='rbf', opt_meth='genetic',
        ...
        logger.info(f"Estimating imprecision intervals in {n_imp_hyc} hypercubes by surrogate optimization ({opt_meth})...")
        ...
        elif self.imp_foc.shape[0] < N_mcs_ale:  # for continuing on existing results
        ...
        elif self.val_samp_prim.shape[0] < N_mcs_ale:  # for continuing on existing results
        ...
        if self.intp_exceed is None:
            intp_exceed = [0, 0]  # count, distance
        else:
            intp_exceed = self.intp_exceed

        if self.intp_undershot is None:
            intp_undershot = [0, 0]  # count, distance
        else:
    ...
    def optimize_inc(self, stat_fun, n_stat, stat_fun_kwargs={}):
        ...
        stat_fun must accept the following arguments:
            ...
            minmax: indicate if minimal or maximal value is sought for e.g. in case of a probability density estimator (which seems intuitive at first but gets difficult to interpret)
    ...
    def hyc_foc_inds(self, vars_):
        ...
        if self.dim_ex == 'cartesian':
            ...
            # a vacuous extension on all variables would just return a single
            # hyperrcube spanning the supports of all variables
            # in the marques2018propagation it is instead understood as a vacuous extension
            # on all variables but the variable of interest, or in the mixed case,
            # on all variables but a selected set of variables
            # Two options:
            # 1) we have to specify this selected set of variables here as a class variable
            # 2) we can temporarily set the focal sets of all other variables to a single support set
            # in any case, the user has to select these variables and perform some manual intervention
            # it probably won't be automated

            # In terms of the imprecision optimization, one could argue, that the
            # vacuous variables could be ignored in the optimization, but that would mean
            # the RBF surrogate has to be trained without knowlegde of this parameter
            # if it is an insensitive parameter, this could be justified as a kind of noise
            # that in turn means, we can actually just remove the variables from
            # the imprecision optimization
            raise NotImplementedError('Vacuous extension currently not implemented, try removing non-sensitive variables from the analysis.')
        else:
...
def plot_grid(df, output='y'):
    ...
    grid = sns.pairplot(df, hue=output, diag_kind='hist', corner=True, markers='.', plot_kws={'s':25, 'hue_norm':hue_norm}, palette='cubehelix',)
